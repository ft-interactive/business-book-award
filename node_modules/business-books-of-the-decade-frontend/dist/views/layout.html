...
<html class="no-js" lang="en-GB">
    <head>
        ...
        <!--[if gt IE 8]><!-->
        <link rel="stylesheet" href="{{ site.staticBaseUrl }}styles/2f883e80.main.css">
        <!--<![endif]-->

        <!--[if lte IE 8]>
        <link rel="stylesheet" href="styles/7a472cf1.oldie.css">
        <![endif]-->
    ...
    <body itemscope itemtype="http://schema.org/WebPage">
        ...
        <header class="header">
            <div class="top-bar">
                <div class="container">
                    <div class="header__logo"><a href="{{ router.url('home') }}" class="ft-logo"><img src="{{ site.staticBaseUrl }}images/brand-ft-masthead.png" alt="Financial Times" /></a></div>
        ...
        <div class="filter-bar filter-bar--hidden clearfix js-filter-bar" id="filterbar">
            ...
            <form class="filter-form js-filter-form" action="/" method="get">
                <div class="filter-form__inner clearfix">
                    <label for="filter" class="filter-form__label text-quiet text-uppercase text-small">
                        <span class="filter-form__filter-by">filter by </span>category
                    </label>
                    <select name="category" id="filter" class="filter-form__select o-ft-forms__field--select js-filter-form-select">
                        <option value="" {% if not category.slug %}selected{% endif %}>Show all</option>
                        {% for category in categories %}
                        <option value="{{ category.slug }}" {% if slug === category.slug %}selected{% endif %}>
                            {{ category.name }}
                        </option>
                        {% endfor %}
                    </select>
        ...
        <script src="{{ site.staticBaseUrl }}scripts/vendor/tipuedrop/tipuedrop_content.js"></script>

        <script src="{{ site.staticBaseUrl }}scripts/971bf15e.bottom.js"></script>
        {#
        ...
        <script>
            ...
            (function() {
              ...
              _gaq.push(['_setAccount', 'UA-35229645-1']);
              _gaq.push(['_trackPageview']);
              _gaq.push(['_setDomainName', 'ig.ft.com']);
              _gaq.push(['_setCookiePath', '/']);

              if (!!window['User'] && window.User.id) {  
                _gaq.push(['_setCustomVar', 1, 'User', User.id, 1]);
              }

              if (window.analytics) {
                window.analytics.timing = function timing(category, variable, time) {
