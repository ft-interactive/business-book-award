...
data.forEach(function (row){
  ...
  list.forEach(function (d) {
    ...
    row.genres.push({name: name, slug: slug});
    genres[slug].push(row);
  });
  delete row.genre;
});
...
function findBooksByGenreName(name, callback) {
  setTimeout(function() {
    callback(null, genres[name] || []);
  }, 20);
}
...
module.exports = function() {
  return {
    find: {
      ...
      booksByGenreName: thunkify(findBooksByGenreName)
    }
  }
};
