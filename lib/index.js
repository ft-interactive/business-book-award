...
module.exports = function (options) {
  ...
  var pageCacheControl = cacheControl({maxage: 60 /* 1 minute */ });
  var data = dataService();

  app.get('home', '/', data, nav, require('./controllers/home'), pageCacheControl);
  app.get('year', '/books/:year', data, nav, require('./controllers/year'), pageCacheControl);
  app.get('book', '/books/:year/:rank/:slug', data, nav, require('./controllers/book'), pageCacheControl);
  app.get('category', '/categories/:slug', data, nav, require('./controllers/category'), pageCacheControl);
  ...
  var apiCacheControl = cacheControl({maxage: 60 * 60 * 24 /* 24hrs */});

  var filter = require('koa-json-filter')();

  app.get('/v1/categories', filter, data, api.category.list, apiCacheControl);
  app.get('/v1/categories/:slug', filter, data, api.category.view, apiCacheControl);
  app.get('/v1/years', filter, data, api.year.list, apiCacheControl);
  app.get('/v1/years/:year', filter, data, api.year.view, apiCacheControl);
  app.get('/v1/books', filter, data, api.book.list, apiCacheControl);
  app.get('/v1/books/:slug', filter, data, api.book.list, apiCacheControl);
